...
function sendEvent(socket: WebSocket, event: string, data: unknown) {
  send(socket, {
    type: "event",
    event,
    data,
...
function getTransport(peer: PeerState, transportId: string) {
...
function getProducer(peerId: string, producerId: string) {
...
function getAllOtherProducers(roomId: string, peerId: string) {
...
async function handleJoinRoom(socket: WebSocket, requestId: string | undefined, data: Record<string, unknown>) {
  ...
  (socket as WebSocket & { peerId?: string }).peerId = peerId;
...
async function handleCreateWebRtcTransport(
  socket: WebSocket,
  requestId: string | undefined,
  data: Record<string, unknown>,
) {
...
async function handleConnectTransport(
  ...
  const dtlsParameters = data.dtlsParameters as DtlsParameters | undefined;
...
async function handleProduce(
  ...
  const peerId = String(data.peerId ?? "");
  const transportId = String(data.transportId ?? "");
  const kind = data.kind as MediaKind | undefined;
  const rtpParameters = data.rtpParameters as RtpParameters | undefined;
...
async function handleConsume(
  ...
  const rtpCapabilities = data.rtpCapabilities as RtpCapabilities | undefined;
...
async function handleRequest(socket: WebSocket, message: RequestMessage) {
...
async function bootstrap() {
  ...
  wss.on("connection", (socket) => {
    socket.on("message", async (rawMessage) => {
      try {
        const message = JSON.parse(rawMessage.toString()) as RequestMessage;
        await handleRequest(socket, message);
      } catch (error) {
        const parsed = (() => {
          try {
            return JSON.parse(rawMessage.toString()) as Partial<RequestMessage>;
          } catch {
            return {};
          }
        })();
    ...
    socket.on("close", () => {
      const peerId = (socket as WebSocket & { peerId?: string }).peerId;
      if (peerId) {
        cleanupPeer(peerId);
      }
    });
